<div id="toPrint"
    ...
    x-data="{showModal : @entangle('showModal'), showError : @entangle('showError'),show_Banner :@entangle('showBanner')}"
                ...
                <div>
                    ...
                    <div class="mt-3 text-center sm:mt-5">
                        ...
                        <div class="mt-2">
                            ...
                            <span class="text-xs text-red-500" x-cloak x-show="showError">Enter a note before saving!
                                Thank you!</span>
                        </div>
                    </div>
                </div>
    ...
    {{-- notif --}}
    <div aria-live="assertive" class="fixed inset-0 flex items-end px-4 py-6 pointer-events-none sm:p-6 sm:items-start" x-cloak x-show="show_Banner">
        <div class="flex flex-col items-center w-full space-y-4 sm:items-end">
            <div
                class="w-full max-w-sm overflow-hidden rounded-lg shadow-lg pointer-events-auto bg-primary-400 ring-1 ring-black ring-opacity-5" x-cloak x-show="show_Banner"
                ...
                <div class="p-4 text-left">
                    <div class="flex items-start">
                        ...
                        <div class="ml-3 w-0 flex-1 pt-0.5 drop-shadow-sm">
                            ...
                            <p class="mt-1 text-xs font-semibold text-white">Anyone with access to the file can view the side note!</p>
                        </div>
    ...
    <a href="{{route('travel-order', ['id'=>3,'isEdit'=>1,'travelOrderID'=>$travelorderID])}}" target="_blank"
        id="printto"
        class="max-w-sm px-4 py-2 font-semibold tracking-wider rounded-full w-sm bg-primary-500 text-primary-200 hover:bg-primary-200 hover:text-primary-500 active:bg-primary-700 active:text-white">Edit
        Travel Order</a>
    @else
    @if ($isSignatory==1)
    <button wire:click="deleteTO('{{ $travel_order->to_type }}')" id="decline"
        ...
        Travel Order</button>
        <button wire:click="deleteTO('{{ $travel_order->to_type }}')" id="approve"
            ...
            Travel Order</button>
            <button x-on:click="showModal = true" id="approve"
                class="max-w-sm px-4 py-2 font-semibold tracking-wider text-white bg-indigo-800 rounded-full w-sm hover:bg-indigo-200 hover:text-primary-500 active:bg-primary-500 active:text-white">Add Side Note</button>
    @else
